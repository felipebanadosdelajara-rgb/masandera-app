...
function renderDashboard() {
  ...
  <div class="dash-charts-row wide">
    ...
    <div class="dash-chart-card">
      ...
      <div class="dash-chart-sub">Unidades vendidas: Pan Tradicional vs Masa Madre</div>
    ...
    <div class="dash-chart-card" style="margin-bottom:20px">
      ...
      <div class="dash-ranking" style="margin-top:16px">
        ${sorted.map(([pId,qty],i)=>{
          ...
          const catLabel = p2?.categoria === 'pan_tradicional' ? 'PT' : 'MM';
  ...
  <div class="dash-chart-card" style="margin-bottom:20px">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px;margin-bottom:20px">
      <div>
        ...
        <div class="dash-chart-sub">${prod.tipo} · <span class="cat-badge cat-${prod.categoria}" style="font-size:10px">${prod.categoria==='pan_tradicional'?'Pan Tradicional':'Masa Madre'}</span> · ${prod.unidad}</div>
      </div>
...
function initDashboardCharts() {
  ...
  data.forEach(p => p.items.forEach(i => {
    ...
    if (prod?.categoria === 'pan_tradicional') ptQty += i.qty; else mmQty += i.qty;
  }));
  ...
  if (cCat) {
    _charts.cat = new Chart(cCat, {
      type: 'doughnut',
      data: {
        labels: ['Masa Madre', 'Pan Tradicional'],
        datasets: [{
          data: [mmQty, ptQty],
