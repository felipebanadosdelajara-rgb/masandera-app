...
// ── Helpers de corte de pedidos ─────────────────────────────

/**
 * Retorna true si el SKU tiene corte configurado y el período ya venció
 * para la fecha de despacho seleccionada.
 *
 * corteAnticipacion: 24 o 48 (horas antes del despacho)
 * corteHora: "HH:MM" — hora límite del día de corte
 *
 * Ejemplo: despacho = 2026-04-16, anticipación = 48h, hora = 07:00
 *   → el pedido debe hacerse antes del 2026-04-14 a las 07:00
 */
function isSkuBloqueado(producto, fechaDespacho) {
  if (!producto.corteAnticipacion) return false;
  if (!fechaDespacho) return false;

  const ahora = new Date();

  // Construir la fecha/hora límite: fechaDespacho - anticipación horas, a la corteHora
  const [hh, mm] = (producto.corteHora || '07:00').split(':').map(Number);
  const despacho = new Date(fechaDespacho + 'T00:00:00');
  // Restar las horas de anticipación para obtener el día de corte
  const limiteDate = new Date(despacho.getTime() - producto.corteAnticipacion * 60 * 60 * 1000);
  // Fijar la hora de corte en ese día
  limiteDate.setHours(hh, mm, 0, 0);

  return ahora >= limiteDate;
}

/** Retorna texto descriptivo del corte para mostrar al usuario. */
function textoCorte(producto, fechaDespacho) {
  if (!producto.corteAnticipacion || !fechaDespacho) return '';
  const [hh, mm] = (producto.corteHora || '07:00').split(':').map(Number);
  const despacho = new Date(fechaDespacho + 'T00:00:00');
  const limiteDate = new Date(despacho.getTime() - producto.corteAnticipacion * 60 * 60 * 1000);
  limiteDate.setHours(hh, mm, 0, 0);
  const dia = limiteDate.toLocaleDateString('es-CL', { weekday:'long', day:'numeric', month:'short' });
  const hora = producto.corteHora || '07:00';
...
function renderNuevoPedido() {
  ...
  const fechaDespachoSel = STATE.draftFecha || (despachos.length ? despachos[0] : '');
  const productGrid = (prods) => prods.map(p => {
    ...
    if (bloqueado) {
      ...
      return `
      <div class="product-card product-card--bloqueado" id="card-${p.id}"
        data-search="${p.nombre.toLowerCase()} ${p.tipo.toLowerCase()} ${p.descripcion.toLowerCase()}"
        data-cat="${p.categoria}"
        style="opacity:.55;pointer-events:none;user-select:none;position:relative">
        <div style="position:absolute;inset:0;border-radius:var(--radius-sm);background:repeating-linear-gradient(-45deg,transparent,transparent 6px,rgba(0,0,0,.03) 6px,rgba(0,0,0,.03) 12px)"></div>
        <div class="product-card-header">
          <div>
            <div class="product-name" style="color:var(--text-light)">${p.nombre}</div>
            <div class="product-unit">${p.unidad}</div>
          </div>
          <span class="cat-badge cat-${p.categoria}">${catLabel}</span>
        </div>
        <div class="product-desc">${p.descripcion}</div>
        <div class="flex-between" style="margin-top:8px">
          <div style="display:inline-flex;align-items:center;gap:6px;padding:5px 10px;background:var(--status-rejected-bg);color:var(--status-rejected);border-radius:20px;font-size:11px;font-weight:600">
            ${icon('clock')} Cerrado · Límite era ${corteTexto}
          </div>
          <div class="product-price" style="color:var(--text-light)">${fmt(price)}</div>
        </div>
      </div>`;
    }
    return `
