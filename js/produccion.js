...
function renderProduccionSub(activos) {
  const canEdit = STATE.user?.rol === 'gerente';
  if (!STATE.prodAjustes) STATE.prodAjustes = {};
  ...
  <div style="display:flex;justify-content:flex-end;gap:10px;margin-bottom:16px">
    ...
    <button class="btn btn-success btn-sm" style="background:var(--success);color:white;border:none" onclick="guardarTodosAjustes()">${icon('check')} Guardar cambios</button>
  </div>
  ${fechas.map(fecha => {
    const pedidos = byIngreso[fecha];
    const ajustesFecha = STATE.prodAjustes[fecha] || {};

    // Inicializar TODOS los SKUs activos en 0 para que siempre aparezcan
    const skuMap = {};
    DATA.productos.filter(p => p.activo !== false).forEach(p => {
      skuMap[p.id] = 0;
    });
    pedidos.forEach(p => {
      p.items.forEach(item => {
        if (skuMap[item.pId] === undefined) skuMap[item.pId] = 0;
    ...
    // Total usando ajustes si existen
    const totalUnids = skuEntries.reduce((s, [pId, qtyBase]) => {
      return s + (ajustesFecha[pId] !== undefined ? ajustesFecha[pId] : qtyBase);
    }, 0);
    ...
    <div class="dispatch-group" style="margin-bottom:24px">
      <div style="display:flex;justify-content:space-between;align-items:center;padding:12px 16px;background:var(--primary-light);border-radius:var(--radius-sm) var(--radius-sm) 0 0">
        ...
        <div style="display:flex;align-items:center;gap:16px">
          ...
          ${canEdit ? `<button class="btn btn-primary btn-sm" style="padding:4px 10px;font-size:11px" onclick="guardarAjustesProduccion('${fecha}')">${icon('check')} Guardar cambios</button>` : ''}
          ${canEdit && Object.keys(ajustesFecha).length > 0 ? `<button class="btn btn-ghost btn-sm" style="padding:4px 10px;font-size:11px;color:var(--warning)" onclick="resetProdAjustes('${fecha}')">${icon('x')} Resetear</button>` : ''}
      ...
      <div style="background:var(--surface);border:1px solid var(--border-light);border-top:none;border-radius:0 0 var(--radius-sm) var(--radius-sm);overflow-x:auto">
        <table class="fact-table">
          ...
          <tbody>
            ${skuEntries.map(([pId, qtyBase]) => {
              const prod = getProducto(pId);
              const ajustado = ajustesFecha[pId] !== undefined;
              const qty = ajustado ? ajustesFecha[pId] : qtyBase;
              return `<tr${ajustado ? ' style="background:var(--primary-faint)"' : ''}>
                <td style="font-family:monospace;font-size:11px;color:var(--text-light)">${prod?.sku||'—'}</td>
                <td style="font-weight:700">${prod?.nombre||'?'}</td>
                <td><span class="cat-badge cat-${prod?.categoria}">${prod?.categoria==='pan_tradicional'?'Pan Tradicional':'Masa Madre'}</span></td>
                <td style="font-size:12px;color:var(--text-light)">${prod?.descripcion||''}${ajustado ? ` <span style="font-size:10px;color:var(--warning);font-weight:600">(base: ${qtyBase})</span>` : ''}</td>
                <td style="text-align:right">
                  ${canEdit
                    ? `<input type="number" min="0" value="${qty}"
                         id="ajuste_${fecha}_${pId}"
                         style="width:70px;text-align:right;padding:4px 6px;border:1px solid var(--border);border-radius:4px;font-size:18px;font-weight:700;color:var(--primary);background:white;font-family:var(--font-display)">`
                    : `<span style="font-family:var(--font-display);font-size:20px;font-weight:700;color:var(--primary)">${qty}</span>`}
                </td>
              </tr>`;
            }).join('')}
          </tbody>
        ...
        <div style="padding:10px 16px;text-align:right;font-size:13px;color:var(--text-light);border-top:1px solid var(--border-light)">
          ...
          ${canEdit ? `<span style="margin-left:12px;font-size:11px;color:var(--text-light)">${icon('edit')} Edita directamente las cantidades</span>` : ''}
        </div>
      </div>
    </div>`;
  }).join('')}`;
...
function guardarTodosAjustes() {
  if (STATE.user?.rol !== 'gerente') {
    showNotif('error', 'Solo el gerente puede guardar cambios.');
    return;
  }
  const activos = PEDIDOS.filter(p => ['aprobado','en_produccion'].includes(p.status));
  const fechas = [...new Set(activos.map(p => p.fechaPedido))];
  fechas.forEach(f => guardarAjustesProduccion(f));
}

function guardarAjustesProduccion(fecha) {
  if (!STATE.prodAjustes) STATE.prodAjustes = {};
  if (!STATE.prodAjustes[fecha]) STATE.prodAjustes[fecha] = {};

  // Leer todos los inputs de esa fecha desde el DOM
  const activos = PEDIDOS.filter(p => ['aprobado','en_produccion'].includes(p.status));
  const skuMap = {};
  DATA.productos.filter(p => p.activo !== false).forEach(p => { skuMap[p.id] = 0; });
  activos.filter(p => p.fechaPedido === fecha).forEach(p => {
    p.items.forEach(item => {
      if (skuMap[item.pId] === undefined) skuMap[item.pId] = 0;
      skuMap[item.pId] += item.qty;
    });
  ...
  Object.keys(skuMap).forEach(pId => {
    const input = document.getElementById(`ajuste_${fecha}_${pId}`);
    if (input) {
      const nuevo = parseInt(input.value) || 0;
      const base = skuMap[pId];
      if (nuevo === base) {
        delete STATE.prodAjustes[fecha][pId];
      } else {
        STATE.prodAjustes[fecha][pId] = nuevo;
      }
    }
  });
...
function resetProdAjustes(fecha) {
  if (STATE.prodAjustes) delete STATE.prodAjustes[fecha];
  saveStorage();
  render();
}
        ...
        <table class="fact-table">
          ...
          <tbody>
            ${lineas.map(l => `<tr>
              ...
              <td><span class="cat-badge cat-${l.categoria}">${l.categoria==='pan_tradicional'?'Pan Tradicional':'Masa Madre'}</span></td>
...
function _htmlProduccionFecha(byIngreso, fechas) {
  const bloques = fechas.map(fecha => {
    ...
    const rows = entries.map(([pId, qty]) => {
      const prod = getProducto(pId);
      const badge = prod?.categoria === 'pan_tradicional'
        ? '<span class="badge badge-br">Pan Tradicional</span>'
...
function _htmlEmpaqueFecha(byDespacho, fechas) {
  const bloques = fechas.map(fecha => {
    ...
    const rows = lineas.map(l => {
      const badge = l.categoria === 'pan_tradicional'
        ? '<span class="badge badge-br">Pan Tradicional</span>'
        : '<span class="badge badge-mm">Masa Madre</span>';
      return `<tr>
...
function renderInventarioSub() {
  ...
  <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;background:var(--surface);border:1px solid var(--border-light);border-radius:var(--radius-sm);padding:8px 14px">
    ...
    ${canEdit
      ...
      : `<span style="font-size:13px;font-weight:600;color:var(--text)">${fechaActual}</span>`}
    ${registroExistente
      ? `<span style="font-size:11px;color:var(--status-approved);font-weight:600;white-space:nowrap">${icon('check')} ${canEdit ? 'Editando registro' : 'Registro guardado'}</span>`
      : `<span style="font-size:11px;color:var(--text-light);white-space:nowrap">${canEdit ? 'Nuevo registro' : 'Sin registro para esta fecha'}</span>`}
    ${canEdit ? `
    ...
    ` : `<span style="margin-left:auto;font-size:11px;color:var(--text-light);font-style:italic">${icon('lock')} Solo lectura</span>`}
  </div>
  ...
  <div style="background:var(--surface);border:1px solid var(--border-light);border-radius:var(--radius-sm);padding:10px 14px;margin-bottom:14px;display:grid;grid-template-columns:repeat(auto-fill,minmax(130px,1fr));gap:4px">
    ${productos.map(prod => {
      ...
      return `<div style="display:flex;align-items:center;gap:6px;background:var(--surface2);border-radius:5px;padding:5px 8px;border-left:2px solid ${prod.categoria==='pan_tradicional'?'#d97706':'var(--primary-light-border,#c49a6c)'}">
        ...
        ${canEdit
          ...
          : `<span style="width:50px;text-align:right;font-size:13px;font-weight:700;color:${qty>0?'var(--primary)':'var(--text-light)'}">${qty !== '' ? qty : '—'}</span>`}
      </div>`;
    }).join('')}
  </div>
