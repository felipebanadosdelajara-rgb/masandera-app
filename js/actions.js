...
function enviarPedido() {
  ...
  // Validar corte de pedidos: bloquear si algún SKU ya cerró para la fecha seleccionada
  const fechaDespachoVal = document.getElementById('draftFecha')?.value || STATE.draftFecha;
  const itemsBloqueados = items.filter(i => {
    const prod = DATA.productos.find(p => p.id === i.pId);
    return prod && isSkuBloqueado(prod, fechaDespachoVal);
  });
  if (itemsBloqueados.length > 0) {
    const nombres = itemsBloqueados.map(i => {
      const prod = DATA.productos.find(p => p.id === i.pId);
      return prod ? prod.nombre : i.pId;
    }).join(', ');
    showNotif('error', `Pedido cerrado para: ${nombres}. El período de pedido ya venció.`);
    return;
  }
